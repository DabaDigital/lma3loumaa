...
**CAPTCHA update:** The new client uses the protected Vercel submission API. Follow [CAPTCHA activation and deployment](../docs/review-protection.md) before publishing this version. The additive quota migration is applied, but production activation still requires Turnstile/server keys and `review-protection.sql`. The direct guest-write behavior below describes the previous submission flow; after cutover, the server creates pending reviews and issues scoped staging uploads instead.
